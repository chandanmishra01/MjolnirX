...
import AmpMigrator from "abis/AmpMigrator.json";
...
const tokens = [
  {
    ...
    iouToken: getContract(CHAIN_ID, "GMT_AMP_IOU"),
    cap: MaxUint256,
    bonus: 0,
  },
  {
    ...
    iouToken: getContract(CHAIN_ID, "XGMT_AMP_IOU"),
    cap: MaxUint256,
    bonus: 0,
  },
  {
    ...
    iouToken: getContract(CHAIN_ID, "GMT_USDG_AMP_IOU"),
  ...
  },
  {
    ...
    iouToken: getContract(CHAIN_ID, "XGMT_USDG_AMP_IOU"),
...
const ampMigratorAddress = getContract(CHAIN_ID, "AmpMigrator");
...
function MigrationModal(props) {
  ...
  const { data: tokenAllowance, mutate: updateTokenAllowance } = useSWR(
    [active, CHAIN_ID, token.address, "allowance", account, ampMigratorAddress],
    {
  ...
  if (amount) {
    baseAmount = amount.mul(token.price).div(ampPrice);
    bonusAmount = baseAmount.mul(token.bonus).div(100);
    totalAmount = baseAmount.add(bonusAmount);

    baseAmountUsd = baseAmount.mul(ampPrice);
    bonusAmountUsd = bonusAmount.mul(ampPrice);
    totalAmountUsd = totalAmount.mul(ampPrice);
  }
  ...
  const onClickPrimary = () => {
    ...
    setIsMigrating(true);
    const contract = new ethers.Contract(ampMigratorAddress, AmpMigrator.abi, library.getSigner());
    contract
        ...
        <div className="MigrationModal-info-box">
          <div className="App-info-row">
            ...
            <div className="align-right">
              {baseAmount &&
                `${formatAmount(baseAmount, 18, 4, true)} AMP ($${formatAmount(
                  baseAmountUsd,
                  18 + decimals,
                  2,
                  true
          ...
          {token.bonus > 0 && (
            <div className="App-info-row">
              ...
              <div className="align-right">
                {bonusAmount &&
                  `${formatAmount(bonusAmount, 18, 4, true)} AMP ($${formatAmount(
                    bonusAmountUsd,
                    18 + decimals,
                    2,
                    true
          ...
          {token.bonus > 0 && (
            <div className="App-info-row">
              ...
              <div className="align-right">
                {totalAmount &&
                  `${formatAmount(totalAmount, 18, 4, true)} AMP ($${formatAmount(
                    totalAmountUsd,
                    18 + decimals,
                    2,
                    true
...
export default function Migration() {
  ...
  const { data: migratedAmounts, mutate: updateMigratedAmounts } = useSWR(
    ["Migration:migratedAmounts", CHAIN_ID, ampMigratorAddress, "getTokenAmounts"],
    {
      fetcher: contractFetcher(library, AmpMigrator, [tokenAddresses]),
    }
  );

  let ampBalance;
  let totalMigratedAmp;
  let totalMigratedUsd;

  if (iouBalances) {
    ampBalance = bigNumberify(0);
    totalMigratedAmp = bigNumberify(0);

    for (let i = 0; i < iouBalances.length / 2; i++) {
      ampBalance = ampBalance.add(iouBalances[i * 2]);
      totalMigratedAmp = totalMigratedAmp.add(iouBalances[i * 2 + 1]);
    }

    totalMigratedUsd = totalMigratedAmp.mul(ampPrice);
  }
  ...
  return (
    <div className="Migration Page">
      ...
      <div className="Migration-note">
        <Trans>Your wallet: {formatAmount(ampBalance, 18, 4, true)}</Trans> AMP
      </div>
      <div className="Migration-note">
        <Trans>
          Please read the{" "}
          <ExternalLink href="https://gambitprotocol.medium.com/gambit-amp-migration-now-live-2ba999d208dd">
            Medium post
          </ExternalLink>{" "}
          before migrating.
        </Trans>
