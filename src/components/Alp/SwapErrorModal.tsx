...
import { getLowestFeeTokenForBuyAlp, InfoTokens, Token } from "domain/tokens";
...
export default function SwapErrorModal({
  ...
  >();
  useEffect(() => {
    const lowestFeeTokenInfo = getLowestFeeTokenForBuyAlp(
      chainId,
      alpAmount,
      alpPrice,
      usdgSupply,
  ...
  }, [chainId, alpAmount, alpPrice, usdgSupply, totalTokenWeights, infoTokens, swapUsdMin, swapToken.address]);
  ...
  return (
    <Modal isVisible={isVisible} setIsVisible={setIsVisible} label={label} className="Error-modal">
      <Trans>
        <p>The pool's capacity has been reached for {swapToken.symbol}. Please use another token to buy ALP.</p>
